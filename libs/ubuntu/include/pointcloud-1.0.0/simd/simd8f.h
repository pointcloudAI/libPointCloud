...
// ----------------------------------------------------------------------------------------
    ...
    //pointcloud.ai 
    inline simd8f atan2(const simd8f& y,const simd8f& x)
    {
            //! For convenience
          float a[8];
          float b[8];
          ...
          a[0] = std::atan2(b[0], a[0]);
          a[1] = std::atan2(b[1], a[1]);
          a[2] = std::atan2(b[2], a[2]);
          a[3] = std::atan2(b[3], a[3]);
          a[4] = std::atan2(b[4], a[4]);
          a[5] = std::atan2(b[5], a[5]);
          a[6] = std::atan2(b[6], a[6]);
          a[7] = std::atan2(b[7], a[7]);
...
// ----------------------------------------------------------------------------------------
    ...
    inline simd8i round(const simd8f& item)
    {
#ifdef DLIB_HAVE_AVX
        __m256 x = _mm256_round_ps(item,_MM_FROUND_TO_NEAREST_INT |_MM_FROUND_NO_EXC);
        return _mm256_cvttps_epi32(x);
#else
        return simd8i(round(item.low()),
                      round(item.high()));
#endif
    }
